...
import static com.github.manolo8.darkbot.extensions.plugins.PluginHandler.PLUGIN_UPDATE_FOLDER;
...
public class PluginUpdater implements API.Singleton {
    ...
    public void scheduleUpdateChecker() {
        ...
        new Thread(() -> {
            while (true) try {
                String[] files = PLUGIN_UPDATE_FOLDER.list();
                if (files != null && files.length != 0) {
                    pluginHandler.updatePluginsSync();
                    TrayButton.showMessage("Updated", String.join("\n", files));
                }
                Thread.sleep(100);
